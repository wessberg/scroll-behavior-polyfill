 ...
 * @param {number} y
 */
export function handleScrollMethod(
 ...
 * @param {ScrollMethodName} kind
 */
function onScrollWithOptions(
  options: Required<ScrollToOptions>,
  element: Element | Window,
  kind: ScrollMethodName
): void {
  ...
  if (behavior == null || behavior === "auto") {
    getOriginalScrollMethodForKind(kind, element).call(
      element,
      options.left,
      options.top
    );
    ...
    smoothScroll(
      getSmoothScrollOptions(element, options.left, options.top, kind)
    );
  }
}
 ...
 */
function getScrollToOptionsWithValidation(
  optionsOrX?: number | ScrollToOptions,
  y?: number
): Required<ScrollToOptions> {
