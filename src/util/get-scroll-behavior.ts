...
const styleAttributePropertyNameRegex = new RegExp(
  `${styleAttributePropertyName}:\\s*([^;]*)`
);
...
export function getScrollBehavior(
  inputTarget: Element | HTMLElement | Window,
  ...
  if ("style" in target) {
    ...
    const scrollBehaviorPropertyValue = target.style[
      styleDeclarationPropertyName
    ] as ScrollBehaviorRawValue;
    // Return it if it is given and has a proper value
    if (
      scrollBehaviorPropertyValue != null &&
  ...
  if (value == null) {
    ...
    const computedStyleValue = computedStyle.getPropertyValue(
      "scrollBehavior"
    ) as ScrollBehaviorRawValue;
